...
@Component({
  selector: 'app-establishment-estates',
  templateUrl: './establishment-estates.component.html'
})
export class EstablishmentEstatesComponent {
  ...
  columnsInfos: ColumnInformation[] = [
    {name: 'reference',   title: 'Référence',     filterType: FilterType.TEXT,   filterDisabled: false, routerLink: () => 'estates/:id'},
  ...
  createAction = {label: 'Créer un bien', disabled: false, hidden: false, create: () => this.createEstate()};
  ...
  setFilterData(filter: string) {
    ...
    this.estatesService.getFromEstablishment(this.full.establishment.uuid).subscribe((estates) => {
      this.estates = estates;
      const dataSource = new MatTableDataSource<EstablishmentEstateTableLine>(this.buildTableLine(estates));
  ...
  buildTableLine(estates: Estate[]): EstablishmentEstateTableLine[] {
    let i = 1;
    return estates && estates.length ? estates.map((estate) => {
      ...
      const data: EstablishmentEstateTableLine = {
        id: estate.id,
  ...
  clickEvent(actionEvent: ActionEvent) {
    const estateTableLine: EstablishmentEstateTableLine = actionEvent.event;
  ...
  createEstate() {
    const data: EstateCreateData = {
      defaultEstablishment: this.full.establishment ? EstablishmentUtils.buildEstablishment(this.full.establishment) : null,
      defaultAccount: this.full.account
    };
  ...
  getRouterLink = (columnName: string, row: EstablishmentEstateTableLine) => {
...
export interface EstablishmentEstateTableLine extends TableLine {
  reference: string;
  name: string;
  estateType: string;
  addressName: string;
