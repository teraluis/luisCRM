...
@Component({
  selector: 'app-account-estates',
  templateUrl: './account-estates.component.html'
})
export class AccountEstatesComponent {
  ...
  columnsInfos: ColumnInformation[] = [
    ...
    {name: 'ownerName',   title: 'Établissement', filterType: FilterType.TEXT,   filterDisabled: false, routerLink: () => 'establishments/:ownerId'},
  ...
  createAction = {label: 'Ajouter un bien', disabled: true, hidden: true, create: () => this.createEstate()}; // TODO manage EstateOwnerType.ACCOUNT ?
  ...
  setFilterData(filter: string) {
    ...
    this.estatesService.getFromAccount(this.account.uuid).subscribe((estates) => {
      ...
      const dataSource = new MatTableDataSource<AccountEstateTableLine>(this.buildTableLine(estates));
  ...
  buildTableLine(estates: Estate[]): AccountEstateTableLine[] {
    let i = 1;
    return estates && estates.length ? estates.map((estate) => {
      ...
      const data: AccountEstateTableLine = {
        id: estate.id,
        ...
        ownerName: EstateUtils.getOwnerType(estate) === EstateOwnerType.ESTABLISHMENT ? EstateUtils.getOwnerName(estate) : 'Lié à ce compte',
        ownerId: EstateUtils.getOwnerId(estate),
  ...
  clickEvent(actionEvent: ActionEvent) {
    const estateTableLine: AccountEstateTableLine = actionEvent.event;
  ...
  getRouterLink = (columnName: string, row: AccountEstateTableLine) => {
    if (columnName === 'ownerName') {
      if (row.ownerType === EstateOwnerType.ESTABLISHMENT) {
...
export interface AccountEstateTableLine extends TableLine {
  reference: string;
  name: string;
  estateType: string;
  ownerName: string;
