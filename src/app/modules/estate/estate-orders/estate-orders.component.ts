import {Component, EventEmitter, Input, OnInit} from "@angular/core";
import {ColumnInformation, FilterType, TableLine, TableOption, TableSearchListInterface
} from "../../table-search-list/table-search-list.component";
import {MatDialog} from "@angular/material/dialog";
import {MatTableDataSource} from "@angular/material/table";
import {SidePanelType} from "../../../services/front/sidepanel.service";
import {Order, OrdersService} from "../../../services/backend/orders.service";
import {OrderStatus, OrderUtils} from "../../utils/order-utils";
import {EstateOwnerType, EstateUtils} from "../../utils/estate-utils";
import {AccountUtils} from "../../utils/account-utils";
import {Estate} from "../../../services/backend/estates.service";

@Component({
  selector: 'app-estate-orders',
  templateUrl: './estate-orders.component.html'
})
export class EstateOrdersComponent implements OnInit, TableSearchListInterface {

  @Input() estate: Estate;
  @Input() disabled: boolean;

  ownerType: EstateOwnerType;
  isEmpty = false;
  // Datagrid implementation
  isLoading = new EventEmitter<boolean>();
  options: TableOption[] = [TableOption.REORDER, TableOption.PREVIEW];
  displayedColumns = ['name', 'clientName', 'market', 'referenceNumber', 'status', 'created'];
  hiddenColumns = [];
  columnsInfos: ColumnInformation[] = [
    {name: this.displayedColumns[0], title: 'Commande', filterType: FilterType.TEXT, filterDisabled: false, routerLink: () => 'orders/:id', maxWidthPx: 120},
    {name: this.displayedColumns[1], title: 'Client', filterType: FilterType.TEXT, filterDisabled: false, routerLink: (e) => (e.isEstablishment ? 'establishments' : 'comptes') + '/:clientId'},
    {name: this.displayedColumns[2], title: 'Marché', filterType: FilterType.MARKET, filterDisabled: false, routerLink: () => 'market/:marketId'},
    {name: this.displayedColumns[3], title: 'Bon de commande', filterType: FilterType.TEXT, filterDisabled: false},
    {name: this.displayedColumns[4], title: 'Statut', filterType: FilterType.STATUS, filterDisabled: false},
    {name: this.displayedColumns[5], title: 'Date de création', filterType: FilterType.DATE, filterDisabled: true}
  ];
  maxRows = 10;
  sidePanelType = SidePanelType.ORDER;
  dataSourceChange = new EventEmitter<any>();

  constructor(protected ordersService: OrdersService,
              protected dialog: MatDialog) {
  }

  ngOnInit() {
    this.ownerType = EstateUtils.getOwnerType(this.estate);
    if (this.ownerType === EstateOwnerType.INDIVIDUAL) {
      this.displayedColumns = this.displayedColumns.filter(c => c !== 'market');
    }
  }

  setPageData(page: number) {
    this.setFilterData('');
  }

  setFilterData(filter: string) {
    this.isLoading.emit(true);
    this.ordersService.getFromEstate(this.estate.id).subscribe((orders) => {
      const dataSource = new MatTableDataSource<EstateOrderTableLine>(this.buildOrderTableLine(orders));
      this.isEmpty = dataSource.data.length === 0;
      this.dataSourceChange.emit({dataSource: dataSource, filter: filter});
      this.isLoading.emit(false);
    });
  }

  buildOrderTableLine(orders: Order[]) {
    let i = 1;
    return orders.map(order => {
      const data: EstateOrderTableLine = {
        id: order.uuid,
        name: order.name,
        clientName: order.establishment ? order.establishment.establishment.name : AccountUtils.getName(order.account),
        clientId: order.establishment ? order.establishment.establishment.uuid : order.account.uuid,
        isEstablishment: !!order.establishment,
        market: order.market ? order.market.name : 'N/A',
        marketId: order.market ? order.market.uuid : 'N/A',
        status: OrderUtils.getStatus(order.status),
        position: i,
        referenceNumber: order.referenceNumber,
        created: order.created
      };
      i++;
      return data;
    });
  }

  getRouterLink = (columnName: string, row: EstateOrderTableLine) => {
    switch (columnName) {
      case 'name':
        return ['/orders', row.id];
      case 'clientName':
        return this.ownerType === EstateOwnerType.ESTABLISHMENT ? ['/establishments', row.clientId]
          : (this.ownerType === EstateOwnerType.ACCOUNT ? ['/comptes', row.clientId]
            : (this.ownerType === EstateOwnerType.INDIVIDUAL ? ['/individuals', row.clientId] : null));
      case 'market':
        return ['/market', row.marketId];
      default:
        return ['/orders', row.id];
    }
  }

  getStatusList = () => {
    const result = [];
    Object.values(OrderStatus).forEach(e => {
      if (!isNaN(Number(e))) {
        result.push({name: e, value: OrderUtils.getStatusName(e)});
      }
    });
    return result;
  }

  getStatusColor = (key: number) => {
    return OrderUtils.getStatusColor(key);
  }

  getStatusName = (key: number) => {
    return OrderUtils.getStatusName(key);
  }
}

export interface EstateOrderTableLine extends TableLine {
  name: string;
  referenceNumber: string;
  clientName: string;
  clientId: string;
  market: string;
  marketId: string;
  status: number;
  created: number;
  isEstablishment: boolean;
}
